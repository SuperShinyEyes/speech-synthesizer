...
(defvar localdir_multisyn_dir (cdr (assoc 'localdir_multisyn-gam voice-locations)))
...
(if (not (member_string 'unilex-gam (lex.list)))
    (load (path-append lexdir "unilex/" (string-append 'unilex-gam ".scm"))))
...
(make_voice_definition 'localdir_multisyn-gam
		       16000
		       'voice_localdir_multisyn_configure
		       unilex-gam-backoff_rules
		       localdir_data_dir
...
(define (voice_localdir_multisyn_configure_pre voice)
 ...
 Set up the current voice to be female British English (Localdir) for
...
(define (voice_localdir_multisyn_configure voice)
  "(voice_localdir_multisyn_configure voice)
 Set up the current voice to be female American English (Localdir) for
 the multisyn unitselection engine."
  (let (this_voice)
...
(proclaim_voice
 'localdir_multisyn-gam
 '((language english)(gender female)(dialect american)
   (description "Localdir multisyn unitsel voice.")))


(provide 'localdir_multisyn-gam)
